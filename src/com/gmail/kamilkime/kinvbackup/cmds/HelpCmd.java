...
public class HelpCmd {
	...
	public void help(CommandSender sender, Command cmd, String l, String args[]){
		...
		sender.sendMessage(StringUtils.color(" &8&l» &a/kinv show <nick> <worldName> &8- &7Shows player's backuped items"));
		sender.sendMessage(StringUtils.color(" &8&l» &a/kinv reload &8- &7Reloads config file"));
		if(Main.SET.enableInfoCommand) sender.sendMessage(StringUtils.color(" &8&l» &a/kinv info &8- &7Little info about the plugin :)"));
		sender.sendMessage(StringUtils.color("                    &6&l--------------------"));
	}
	...
	public void pomoc(CommandSender sender, Command cmd, String l, String args[]){
		...
		sender.sendMessage(StringUtils.color(" &8&l» &a/kinv pokaz <nick> <worldName> &8- &7Pokazuje zapisane itemy gracza"));
		sender.sendMessage(StringUtils.color(" &8&l» &a/kinv reload &8- &7Przeladowuje config"));
		if(Main.SET.enableInfoCommand) sender.sendMessage(StringUtils.color(" &8&l» &a/kinv info &8- &7Male info o pluginie :)"));
		sender.sendMessage(StringUtils.color("                    &6&l--------------------"));
	}
}